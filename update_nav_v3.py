...
for filename in files:
    try:
        # Try to read as utf-8-sig first (handles BOM)
        try:
            with open(filename, 'r', encoding='utf-8-sig') as f:
                content = f.read()
            encoding_to_save = 'utf-8-sig'
        except UnicodeDecodeError:
            # If that fails, read with surrogateescape to preserve bytes
            with open(filename, 'r', encoding='utf-8', errors='surrogateescape') as f:
                content = f.read()
            encoding_to_save = 'utf-8'
        ...
        with open(filename, 'w', encoding=encoding_to_save, errors='surrogateescape' if encoding_to_save == 'utf-8' else 'strict') as f:
            f.write(content)
